...
const char *status_string(Status s) noexcept {
  ...
  case Status::Success:
    return "Success";
  case Status::ErrorInvalidArgument:
    return "Invalid argument";
  case Status::ErrorBufferTooSmall:
    return "Buffer too small";
  case Status::ErrorBadAlignment:
    return "Bad alignment";
  case Status::ErrorCudaError:
    return "CUDA error";
  case Status::ErrorCorruptData:
    return "Corrupt data";
  case Status::ErrorCorruptHeader:
    return "Corrupt header";
  case Status::ErrorUnsupportedFormat:
    return "Unsupported format";
  case Status::ErrorInternal:
...
size_t compress_bound(size_t input_size,
  ...
  constexpr size_t header_overhead =
      sizeof(detail::FileHeader) + 4096; // tables + descriptors
  return input_size * 2 + header_overhead;
}

size_t get_decompressed_size(const uint8_t *compressed_data,
...
// =============================================================================
CompressResult Codec::compress(const uint8_t *src, size_t src_size,
  ...
  const size_t bound = compress_bound(src_size, opts);
  if (dst_capacity < bound) {
    // Try anyway — actual compressed size may fit
  }
  ...
  if (num_blocks == 0) {
    num_blocks =
        static_cast<uint32_t>((src_size + detail::DEFAULT_BLOCK_SIZE - 1) /
                              detail::DEFAULT_BLOCK_SIZE);
    if (num_blocks == 0)
      num_blocks = 1;
  }
  ...
  // --- Build symbol table from sampled data ---------------------------
  // For now we use a single table for all blocks (can be extended to
  // per-super-block tables like fsst-gpu does).
  constexpr size_t SAMPLE_TARGET = 65536;
  const size_t sample_size = std::min(src_size, SAMPLE_TARGET);
  detail::DecoderTable decoder{};
  detail::EncoderTable *encoder =
      detail::build_encoder_table(src, sample_size, &decoder);
  if (!encoder) {
  ...
  const size_t table_serialized_size =
      decoder.export_to(table_buf, sizeof(table_buf));
  ...
  // We'll compute compressed_size after encoding all blocks.
  // For now, skip ahead and fill in block descriptors + data.
  ...
  // --- Encode each block (CPU fallback for now) -----------------------
  // TODO: Replace with GPU kernel pipeline from fsst-gpu
  ...
  for (uint32_t b = 0; b < num_blocks; ++b) {
    ...
    // Simple CPU FSST encoding: scan input, match symbols from decoder table
    // (greedy longest-match). This is the reference implementation; the GPU
    // kernel in encoder.cu will replace this for production throughput.
    ...
    while (in_pos < this_block_size) {
      ...
      uint8_t best_code = detail::ESC_CODE;
      uint8_t best_len = 1;

      for (uint16_t code = 0; code < detail::MAX_SYMBOLS; ++code) {
        const uint8_t sym_len = decoder.len[code];
        if (sym_len == 0 || sym_len > (this_block_size - in_pos))
          continue;
        if (sym_len <= best_len && best_code != detail::ESC_CODE)
          continue;

        // Compare symbol bytes (little-endian in symbol[])
        const uint8_t *sym_bytes =
            reinterpret_cast<const uint8_t *>(&decoder.symbol[code]);
        if (std::memcmp(block_src + in_pos, sym_bytes, sym_len) == 0) {
          best_code = static_cast<uint8_t>(code);
          best_len = sym_len;
        }
      }

      if (best_code == detail::ESC_CODE) {
        // Escape: emit ESC + literal byte
        block_dst[out_pos++] = detail::ESC_CODE;
        block_dst[out_pos++] = block_src[in_pos];
        in_pos += 1;
      } else {
        block_dst[out_pos++] = best_code;
        in_pos += best_len;
      }
    }
  ...
  std::memcpy(dst + descriptors_offset, descriptors.data(),
              num_blocks * sizeof(detail::BlockDescriptor));
...
// =============================================================================
DecompressResult Codec::decompress(const uint8_t *src, size_t src_size,
                                   uint8_t *dst, size_t dst_capacity,
  ...
  const size_t bd_end =
      bd_off + hdr.num_blocks * sizeof(detail::BlockDescriptor);
  if (bd_end > src_size) {
    result.status = Status::ErrorCorruptData;
    return result;
  }
  auto *descriptors =
      reinterpret_cast<const detail::BlockDescriptor *>(src + bd_off);
...
// =============================================================================
CompressResult Codec::compress_device(const uint8_t * /*d_src*/,
                                      size_t /*src_size*/, uint8_t * /*d_dst*/,
                                      size_t /*dst_capacity*/,
                                      const CompressOptions & /*opts*/) const {
...
DecompressResult
Codec::decompress_device(const uint8_t * /*d_src*/, size_t /*src_size*/,
                         uint8_t * /*d_dst*/, size_t /*dst_capacity*/,
                         const DecompressOptions & /*opts*/) const {
  // TODO: Implement GPU decompression dispatching to layout-specific kernels
  return DecompressResult{.status = Status::ErrorInternal,
                          .decompressed_size = 0};
}
...
// =============================================================================
std::vector<uint8_t> Codec::compress(std::string_view input,
                                     const CompressOptions &opts) const {
  const size_t bound = compress_bound(input.size(), opts);
  std::vector<uint8_t> buf(bound);
  auto res = compress(reinterpret_cast<const uint8_t *>(input.data()),
                      input.size(), buf.data(), buf.size(), opts);
...
std::string Codec::decompress_string(const uint8_t *src, size_t src_size,
                                     const DecompressOptions &opts) const {
  size_t orig_size = get_decompressed_size(src, src_size);
  if (orig_size == 0)
    return {};
  std::string out(orig_size, '\0');
  auto res = decompress(src, src_size, reinterpret_cast<uint8_t *>(out.data()),
                        out.size(), opts);
  if (res.status != Status::Success)
    return {};
...
const char *gsst_version_string(void) { return "0.1.0"; }
