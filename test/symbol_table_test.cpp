...
TEST(SymbolTableTest, BuildFromSample) {
    ...
    auto* encoder = build_encoder_table(reinterpret_cast<const uint8_t*>(sample.data()), sample.size());

    ASSERT_NE(encoder, nullptr);
    const auto& decoder = encoder->decoder;
...
TEST(SymbolTableTest, ExportImportRoundtrip) {
    ...
    auto* encoder = build_encoder_table(reinterpret_cast<const uint8_t*>(sample.data()), sample.size());
    ASSERT_NE(encoder, nullptr);
    const auto& original = encoder->decoder;
    ASSERT_NE(encoder, nullptr);
...
TEST(SymbolTableTest, NullInputReturnsNull) {
    EXPECT_EQ(build_encoder_table(nullptr, 0), nullptr);
    EXPECT_EQ(build_encoder_table(nullptr, 100), nullptr);
    uint8_t data[] = {1, 2, 3};
    EXPECT_EQ(build_encoder_table(data, 0), nullptr);
}
